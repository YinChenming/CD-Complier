...
TAC *declare_var(char *name) { return mk_tac(TAC_VAR, mk_var(name), NULL, NULL); }
...
TAC *declare_para(char *name) { return mk_tac(TAC_FORMAL, mk_var(name), NULL, NULL); }
...
void out_sym(FILE *f, SYM *s) { out_str(f, "%p\t%s", s, s->name); }
