...
namespace cfg {
    ...
    public:
        ...
        [[nodiscard]] bool defines_a() const {
            if (!tac_ || !tac_->a) return false;
            if (tac_->op == TAC_STORE) return false;
            if (tac_->op == TAC_VAR) return true;
            if (tac_->op == TAC_INPUT) return true;
            if (tac_->op == TAC_COPY) return true;
            if (tac_->op == TAC_ADDR) return true;
        ...
        [[nodiscard]] bool has_memory_effect() const {
            if (!tac_) return false;
            return tac_->op == TAC_STORE || tac_->op == TAC_DEREF || tac_->op == TAC_CALL || tac_->op == TAC_INPUT;
        }
