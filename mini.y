...
%%
    ...
    | CASE INTEGER ':'
    {
        $$=mk_case(mk_const(atoi($2), SYM_VAL_DEFAULT));
    }
    | DEFAULT ':'
    {
        $$=mk_default();
    ...
    | '*' store_assignment_statement
    {
        ...
        const EXP *deref_exp = do_un(TAC_DEREF, mk_exp(NULL, (SYM *)underef_sym, NULL));
    ...
    | FOR '(' for_exp_opt ';' expression ';' for_exp_opt ')' block
    {
        ...
        for (const TAC* t=$9;t;t=t->prev) {
            if (t->op == TAC_GOTO && t->a && t->a->type == SYM_LABEL && t->a->value_type == SYM_LABEL_CONTINUE) {
    ...
    switch_statement: SWITCH '(' expression ')' block
    {
        $$=do_switch($3, $5);
    }
    ;
